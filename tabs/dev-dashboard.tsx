...
function DevDashboard() {
  const dashboardPort = usePort<DashboardRequestBody, DashboardResponseBody>(
    "devdashboard"
  )
  ...
  //   })
  // });
  const [visibleTypes, setVisibleTypes] = useState(["BROWSE_VIEW"])
  ...
  const [isWorkingShareStatus, setIsWorkingShareStatus] =
    useState<ShareStatusType>({ current: 0, total: 0 })
  ...
  annotationsPort.listen((data) => {
    if (
      data.actionType === GET_ANNOTATIONS &&
  ...
  ioPort.listen((data) => {
    ...
    if (
      data.actionType === DELETE_ALL_DATA &&
      data.result.status === "success"
    ) {
      setIsWorking(false)
      requestPreviewUpdate()
        ...
        try {
          ...
          if (recordKeys.every((k) => k in data)) {
            const confirmed = confirm(
              "L'entièreté de l'historique d'activité, des paramètres et des annotations actuelles va être écrasée et remplacée par les données du fichier versé. Confirmer ?"
            )
            if (confirmed) {
              // newActivity = data;
              setIsWorking(true)
  ...
  const handleDuplicateTodayForPastDays = async (numberOfDays = 100) => {
    if (
      !confirm(
        `Vous allez définitivement modifier l'historique en multipliant sa taille par ${numberOfDays} dans le cadre du test de charge. Continuer ?`
      )
    ) {
      return
    }
  ...
  return (
    <div className="DevDashboard">
      <h1>
        <strong>lore selfie</strong> | dashboard de développement
      </h1>
      <div className="about">
        <h3>Qu'est-ce que cette page ?</h3>
        <p>
          Cette page est dédiée aux développeur·euses qui désirent visualiser,
          télécharger et manipuler les données créées avec l'extension lore
          selfie installée sur ce navigateur à des fins de test technique et
          d'expérimenation.
        </p>
        <p>
          Ne l'utilisez que si vous savez ce que vous faites, au risque de
          compromettre vos données patiemment constituées !
        </p>
      </div>
      <div className="ui">
        <div className="ui-section">
          ...
          <button
            className="important-button danger"
            onClick={() => {
              const confirmed = confirm(
                "Supprimer toutes les données enregistrées par lore selfie ?"
              )
              if (confirmed) {
        ...
        <div className="ui-section">
          ...
          <FileDrop onTargetClick={onTargetClick} onDrop={onFileInputChange}>
            Charger des données existantes
          </FileDrop>
        </div>
        <div className="ui-section">
          <h2>
            Tests de charge (pour évaluer la robustesse de l'extension à de
            grands volumes)
          </h2>
          <ul>
            <li>
              <button
                className="important-button ok full-width"
            ...
            <li>
              <button
                className="important-button warning full-width"
            ...
            <li>
              <button
                className="important-button danger full-width"
            ...
            <li>
              <button
                ...
                Dupliquer les données d'aujourd'hui sur les 10000 derniers jours
                (27 ans)
              </button>
            </li>
          </ul>
        ...
        <div className="ui-section">
          ...
          <div>
            <button onClick={() => setVisibleTypes([])}>Cacher tout</button>
          ...
          <h2>
            <span>
              Visualisation de{" "}
              {filteredCount === undefined ? "?" : filteredCount} évènements sur{" "}
              {totalCount === undefined ? "?" : formatNumber(totalCount)} tous
              types d'évènements confondus (page{" "}
              {reverseOrder
                ? paginations.length - currentPreviewPage
          ...
          <div>
            <div className="row">
              <div>
                ...
                <button
                  className="important-button"
                  disabled={isLoadingPreview || currentPreviewPage === 0}
                  onClick={() => setCurrentPreviewPage(currentPreviewPage - 1)}>
                  {"< page précédente"}
                </button>
                <button
                  className="important-button"
                  disabled={
                    isLoadingPreview ||
                    currentPreviewPage >= paginations.length - 1
                  }
                  onClick={() => setCurrentPreviewPage(currentPreviewPage + 1)}>
                  {"page suivante >"}
                </button>
              ...
              <div>
                {paginations.length > 1
                  ? (paginations.length < 10
                      ? paginations
                      : [
                          ...(currentPreviewPage > 6
                            ? [...paginations.slice(0, 5), "..."]
                            : paginations.slice(
                                0,
                          ...
                          ...paginations.slice(
                            currentPreviewPage > 0 ? currentPreviewPage - 1 : 1,
                            currentPreviewPage < 4 ? 5 : currentPreviewPage + 2
                          ),
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            pageNumber !== "..."
                              ? setCurrentPreviewPage(pageNumber as number)
                              : undefined
                          }
                          key={index}
                          ...
                          {pageNumber === "..."
                            ? pageNumber
                            : reverseOrder
                              ? paginations.length - (pageNumber as number)
      ...
      <div className={`worker-loading-container ${isWorking ? "active" : ""}`}>
        Opérations en cours{" "}
        {isWorkingShareStatus
          ? `${isWorkingShareStatus.current + 1} / ${isWorkingShareStatus.total} (${Math.round((isWorkingShareStatus.current / isWorkingShareStatus.total) * 100)}%)`
          : ""}
      </div>
    </div>
  )
