  ...
  switch (actionType) {
    ...
    case DELETE_ANNOTATION:
      if (["collection", "id"].every((k) => payload[k])) {
        ...
        newAnnotations = Object.entries(annotations)
              ...
              Object.entries(collectionItems)
                .map(([itemId, item]: [string, Tag | Expression | Creator]) => {
                  if (
                    ...
                    (item as Creator).links[collection]
                  ) {
                    return [
                      itemId,
                      {
                        ...
                        links: {
                          ...(item as Creator).links,
                          [collection]: (item as Creator).links[
                            collection
                ...
                .reduce(
                  (
                    res,
                    [itemId, item]: [string, Tag | Expression | Creator]
                  ) => ({ ...res, [itemId]: item }),
                  {}
                )
            ]
          ...
          .reduce(
            (res, [collectionId, collectionItems]) => ({
              ...res,
