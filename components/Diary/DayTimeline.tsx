...
export default function DayTimeline({
  ...
  const { tickValues, minDate, maxDate } = useMemo(() => {
    ...
    const niceEndInMs =
      realEndInMs % tickTimespan === 0
        ? realEndInMs
        : realEndInMs - (realEndInMs % tickTimespan) + tickTimespan
    const ticks = []
  ...
  const gutter = 5
  const ticksX = 40
  const marginsStart = width - width * annotationColumnsNames.length * 0.2
  const vizSpaceX: [number, number] = useMemo(
    () => [ticksX + gutter * 4, marginsStart],
    [ticksX, marginsStart, gutter]
  )
  ...
  const vizSequences = useMemo(() => {
    ...
    events.forEach((event) => {
      if (event.type === BROWSE_VIEW && contentsMap.has(event.url)) {
        ...
        if (
          sequences.length &&
          event.injectionId === sequences[sequences.length - 1].injectionId
        ) {
    ...
    sequences = sequences.map((sequence) => {
      const end = sequence.liveRecords.length
        ? new Date(sequence.liveRecords[sequence.liveRecords.length - 1].date)
        : sequence.start
      return {
        ...sequence,
        end,
    ...
    for (let i = 0; i < sequences.length; i++) {
      ...
      computed.forEach((prevComputed) => {
        ...
        if (
          // time conditions
          ((min < prevMax && max > prevMax) || min - prevMin < minYDist) &&
          // same column
  ...
  const numberOfVizColumns: number = useMemo(
    () => Math.max(...vizSequences.map((v) => v.columnIndex)) + 1,
    [vizSequences]
  )
  const xScale = useMemo(
    () => scaleLinear().domain([0, numberOfVizColumns]).range(vizSpaceX),
    [numberOfVizColumns, vizSpaceX]
  )

  const marginsFields: Array<string> = annotationColumnsNames
  const marginsData = marginsFields.map((label, index) => {
    ...
    <svg
      ...
      <g className="ticks y-ticks">
        {tickValues.map(({ value, label }) => {
          ...
          return (
            <g
              className="tick-group y-tick-group"
              key={value}
              ...
              <line x1={ticksX + gutter} x2={width} y1={0} y2={0} />
            </g>
          )
        })}
      </g>
          ...
          return (
            <g
              ...
              <line
                ...
                y2={y2 - y1}
              />
              <circle fill={PLATFORMS_COLORS[platform]} cx={0} cy={0} r={10} />
              <text fill="white" x={0} y={3} fontSize={10} textAnchor="middle">
                {index}
              </text>
            </g>
          )
