...
export default function ExpressionCard({
  ...
  const { tags: linkedTagsIds = [], creators: linkedCreatorsIds = [] } = links
          ...
          {isEdited ? (
            <>
              ...
              <div className="form-subgroup">
                <h4>
                  Variantes (exemples : au pluriel, écrit différemment, etc.)
                </h4>
                <ul>
                  {queries.map(({ id, query }, index) => {
                    const handleBlurOrSubmit = () => {
                      if (query?.trim().length) {
                        ...
                        const prevValue = expression.queries.find(
                          (q) => q.id === id
                        )?.query
              ...
              <div className="form-subgroup">
                ...
                <Select
                  ...
                  placeholder={
                    "Sélectionner ou rechercher des étiquettes que vous avez déjà créées"
                  }
                  isMulti
                />
              </div>
              <div className="form-subgroup">
                ...
                <Select
                  ...
                  noOptionsMessage={() => "Rien à afficher"}
                  placeholder={
                    "Sélectionner ou rechercher des créateurs et créatrices enregistrées"
                  }
                  isMulti
                />
              </div>
            ...
            <div
              className="card-readonly-content"
              ...
              <div>
                ...
                <span>
                  {queries.map((q) => '"' + q.query.trim() + '"').join(", ")}
                </span>
              </div>
              {linkedTagsIds.length ? (
                <div>
                  ...
                  <span>
                    {
                      linkedTagsIds.map((id) => (
                        <TagChip key={id} tag={tags[id]} />
                      ))
                      // .map(id => tags[id]?.name)
                      // .join(', ')
                    }
              ...
              {linkedCreatorsIds.length ? (
                <div>
                  <i>
                    Producteurs de contenus associés :{" "}
                    <span>
