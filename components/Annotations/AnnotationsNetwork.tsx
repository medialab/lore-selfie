...
export const LoadGraph = ({ annotations, channels, legend }) => {
  ...
  useEffect(() => {
    ...
    const channelsMap = channels.reduce((res, chan) => ({ ...res, [chan.id]: chan }), {});

    channels.forEach((channel, index) => {
      graph.addNode(`channel-${channel.id}`, { x: 0, y: index, size: channel.urlsCount, label: `${channel.channelName || channel.channelId} (${channel.platform})`, color: legend.channel.color });
    })
    Object.values(annotations.tags).forEach((tag, index) => {
      graph.addNode(`tag-${tag.id}`, { x: 1, y: index, size: 5, label: `${tag.name} (étiquette)`, color: legend.tag.color });
    })
    Object.values(annotations.creators).forEach((creator, index) => {
      ...
      creator.links.tags.forEach(tagId => {
        ...
        graph.addEdge(`creator-${creator.id}`, nodeId);
        creator.channels.forEach(channelId => {
          graph.updateNodeAttribute(`tag-${tagId}`, 'size', val => val + channelsMap[channelId].urlsCount)
        })
      })
    })
    Object.values(annotations.expressions).forEach((expression, index) => {
      graph.addNode(`expression-${expression.id}`, { x: 3, y: index, size: 3, label: `${expression.name} (expression)`, color: legend.expression.color });
      ...
      expression.links.creators.forEach(creatorId => {
        ...
        creator.channels.forEach(channelId => {
          graph.updateNodeAttribute(`expression-${expression.id}`, 'size', val => val + channelsMap[channelId].urlsCount)
        })
      })
    })
    ...
    >
      {({ measureRef }) => (
        <div ref={measureRef} className="AnnotationsNetwork">
          ...
          >
            <LoadGraph legend={legend} channels={channels} annotations={annotations} />
