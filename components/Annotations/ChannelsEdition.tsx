  ...
  return (
    <section className="ChannelsEdition annotation-form">
      <div className="form-header">
        ...
        <p>
          À regrouper les chaînes par créatrices et créateurs pour les
          visualiser ensemble dans les onglets.
        </p>
      </div>
      ...
      <div className="form-list">
            ...
            {visibleSuggestions.length ? (
              <div className="ui-row">
                <button
                  ...
                  onClick={() => {
                    const newCreators = visibleSuggestions.reduce(
                      (tempCreators, suggestion) => {
                        const { type, items, title } = suggestion
                        if (type === "addition") {
                          const [{ id: creatorId }, { id: newChannelId }] =
                            items
                          const newCreators = {
                            ...tempCreators,
                            [creatorId]: {
                              ...
                              channels: [
                                ...tempCreators[creatorId].channels,
                                newChannelId
                              ]
                            }
                          }
                ...
                <button
                  ...
                  onClick={() => {
                    const newDismissed = new Set(
                      Array.from(dismissedSuggestions)
                    )
                  ...
                  Refuser toutes les suggestions
                </button>
              </div>
            ) : null}
          ...
          <>
            ...
            <ul className="cards-list">
              {Object.values(creators)
                .filter((c) =>
                  creatorSearchTerm.length > 2
                    ? c.name
                        .toLowerCase()
                        .includes(creatorSearchTerm.toLowerCase())
                    : true
                )
                .sort((c1, c2) => {
        ...
        <CollapsibleSection
          ...
          <>
            {Object.values(soloChannels).length ? (
              <div className="ui-row">
                <input
                ...
                <button
                  className="important-button"
                  onClick={() => {
                    const newCreators = Object.values(soloChannels).reduce(
                      (tempCreators, channel) => {
                        const newCreator: Creator = {
                          id: generateId(),
                  ...
                  Associer automatiquement toutes ces chaînes
                </button>
              </div>
            ) : null}
            <ul className="cards-list">
      ...
      <div className="form-footer">
        <button
          ...
          onClick={() => {
            const name = prompt(
              "Quelle est le nom de la créatrice ou du créateur que vous voulez ajouter (note : vous pourrez la modifier ensuite) ?"
            )
            if (name.length) {
              const newCreator: Creator = {
                id: generateId(),
