...
export default function TimePicker({
  ...
  return (
    <div className="TimePicker">
      ...
      <select value={actualValues[0]} onChange={e => handleChangeHours(e.target.value)}>
        {
          Object.values(hoursValues)
          .map(({value, label}) => (
            <option key={value} value={value}>{label}</option>
          ))
        }
      </select>
      <select value={actualValues[1]} onChange={e => handleChangeMinutes(e.target.value)}>
        {
          Object.values(minutesValues)
          .map(({value, label}) => (
            <option key={value} value={value}>{label}</option>
          ))
        }
